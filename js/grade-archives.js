...
async function loadSectionPreview(){
  ...
  try{
    ...
    const { data: rows, error } = await sb
      .from('portfolios')
      .select('student_id, assignment_id, creativity_score, technique_score, composition_score, final_grade, grade_remarks')
    ...
    currentPreviewRows = rows.map(r => {
      ...
      return {
        ...
        assignment: r.assignment_id ? (assignmentMap[r.assignment_id] || 'Untitled Assignment') : 'General Submission',
