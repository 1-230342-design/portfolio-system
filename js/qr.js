...
async function loadPublicWorkFromQR(itemId){
  ...
  try{
    ...
    el.innerHTML = `
      ...
      <h2 style="font-family:'DM Serif Display',serif;font-size:26px;color:var(--dark);margin-bottom:6px;">${esc(item.title)}</h2>
      <div style="font-size:13px;color:var(--text3);margin-bottom:16px;">${esc(subj.name||subj.code||'Multimedia Arts')} &middot; by ${esc((profile&&profile.full_name)||'Artfolio Student')}</div>
